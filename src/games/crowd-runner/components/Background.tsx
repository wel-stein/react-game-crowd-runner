...
// Foggy green hills receding to the horizon, plus the grass ground plane.
export function Background() {
  const hills = useMemo(() => {
    ...
    for (let i = 0; i < 26; i++) {
      ...
      const z = -40 - (i / 26) * 320
      const x = side * (12 + hash(i * 2.1) * 26)
      const s = 8 + hash(i * 4.3) * 16
      arr.push({ x, z, s, shade: 0.35 + hash(i * 7.7) * 0.25 })
    }
    return arr
  }, [])
    ...
    <group>
      ...
      {hills.map((h, i) => (
        <mesh key={i} position={[h.x, h.s * 0.35 - 1, h.z]}>
          ...
          <meshStandardMaterial color={`hsl(120, 45%, ${h.shade * 100}%)`} roughness={1} />
        </mesh>
      ))}
    </group>
  )
